...
export default function Header({ user, tutorMode, ...props }: ComponentProps<'header'> & { user: User | undefined; tutorMode: boolean }) {
  ...
  return (
    <header {...props} className={clsx(props.className, 'sticky top-0 z-[1] border-b-2 border-zinc-300 bg-zinc-200')}>
      <div className='flex items-center px-6 py-4'>
        ...
        <Popover.Root onOpenChange={(open) => (headerStore.showUserPopup = open)} open={headerSnap.showUserPopup}>
          <Popover.Content sideOffset={8} align='end' className='w-64 rounded-xl border-2 border-zinc-400 bg-zinc-200 shadow'>
            {user ? (
              <menu className='py-2'>
                <Link
                  href={'/'}
          ...
          <Popover.Trigger className='rounded-full px-4 py-1 hover:bg-zinc-300'> {user ? user.login : 'Log in'}</Popover.Trigger>
        </Popover.Root>
      </div>
      {routes.some(({ route }) => path.includes(route)) && (
        <nav className={clsx(props.className, 'flex px-4 pb-2')}>
          {routes.map((r) => (
            <Link key={r.route} href={r.route} className={clsx('relative rounded-md px-4 py-1', path.includes(r.route) ? 'bg-zinc-300' : '')}>
              {r.title}
              {path.includes(r.route) && <div className='absolute inset-x-0 top-[calc(100%+theme(size.2))] h-[2px] bg-zinc-500' />}
            </Link>
          ))}
        </nav>
      )}
  ...
  return (
    <article {...props} className={clsx(props.className, 'p-4')}>
      <fieldset className='mb-2'>
        ...
        <input ref={loginInputRef} autoComplete='username' type='text' className='w-full rounded-md bg-zinc-300 px-3 py-2' />
      </fieldset>
      <fieldset className=''>
        <label className='mb-1 block text-sm text-zinc-600'>Password</label>
        <input ref={passwordInputRef} autoComplete='current-password' type='password' className='w-full rounded-md bg-zinc-300 px-3 py-2' />
      </fieldset>
      <button className='mb-4 rounded-full text-xs text-zinc-600 hover:text-zinc-900 hover:underline'>I forgot my password</button>
      <button
